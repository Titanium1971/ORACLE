...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Entry point: shows the WebApp button.
    If user arrived via deep link (/start <token>), we must force a per-message WebApp URL that includes &link_token=...
    to bind the formulaire record. Global/menu WebApp buttons are NOT per-user and would drop the token.
    """
    msg = update.effective_message

    # Rate limit: avoid rapid double-taps
    now = time.time()
    last = context.user_data.get("last_start_time", 0)
    if now - last < 1.2:
        return
    ...
    # Capture deep-link token if present
    start_token = ""
    try:
    ...
    # Build canonical WebApp URL
    webapp_url = f"{WEBAPP_BASE_URL}?api={API_BASE_URL}&v={v}&src=start"

    # If token present, append it and DISABLE menu button so user cannot open a token-less WebApp.
    if start_token:
        context.user_data["link_token"] = start_token
        webapp_url = webapp_url + f"&link_token={urllib.parse.quote_plus(start_token)}"
        logger.info("🔑 START_TOKEN_CAPTURED=%s", start_token)

        # Remove chat menu button to prevent opening a generic WebApp without token
        try:
            await context.bot.set_chat_menu_button(chat_id=msg.chat_id, menu_button=MenuButtonDefault())
            logger.info("🧼 CHAT_MENU_BUTTON_RESET chat_id=%s", msg.chat_id)
        except Exception as e:
            logger.warning("⚠️ set_chat_menu_button reset failed: %s", e)
    else:
        # Keep existing behavior: set WebApp menu button globally + per-chat (generic)
        try:
    ...
    # Per-message button (always). This guarantees token propagation when present.
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("🜂 Je suis prêt", web_app=WebAppInfo(url=webapp_url))]]
    )

    # Copy kept concise
    text = (
        "Velvet Oracle — Antichambre.\n\n"
        "Touchez « Je suis prêt » pour ouvrir le rituel."
    )
    if start_token:
        text = (
            "Velvet Oracle — Antichambre.\n\n"
            "Touchez « Je suis prêt » pour lier votre demande et ouvrir le rituel."
        )
