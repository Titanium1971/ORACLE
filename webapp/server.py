...
def dedup_players_by_telegram_user_id(players_table, telegram_user_id: str):
    """Hard guard: ensure there is only ONE players record per telegram_user_id.
    If duplicates exist (race condition / double calls), we keep a canonical record and neutralize others.
    Returns canonical record (dict) or None.
    """
    tg = str(telegram_user_id or "").strip()
    if not tg:
        return None

    formula = "{telegram_user_id}='" + _airtable_escape(tg) + "'"
    res = airtable_list_records(players_table, params={"filterByFormula": formula, "maxRecords": 100})
    if not res.get("ok"):
        return None
    ...
    # Choose canonical: prefer one with status ACTIVE, otherwise most recently modified if field exists, else first
    def score(rec):
        f = rec.get("fields") or {}
        st = str(f.get("status") or "").upper()
        return (1 if st == "ACTIVE" else 0)

    records_sorted = sorted(records, key=score, reverse=True)
    canonical = records_sorted[0]
    canon_fields = canonical.get("fields") or {}

    # Merge: for any field empty in canonical, take the first non-empty from others (exclude computed/system-ish fields)
    EXCLUDE = set([
        "Name","Autonum","created_time","Created time","Created Time",
        "attempt_id","attempt_label","exam_label","score_percent","time_formatted","duration_pretty",
        "completed_flag","Rand"
    ])
    merged = {}
    for rec in records_sorted[1:]:
        ...
        for k, v in f.items():
            if k in EXCLUDE:
                continue
            if k not in canon_fields or canon_fields.get(k) in (None, "", [], {}):
                if v not in (None, "", [], {}):
                    merged[k] = v
        # also merge link_token specifically if canonical missing it
        if (canon_fields.get("link_token") in (None, "")) and f.get("link_token"):
            merged["link_token"] = f.get("link_token")

    if merged:
        airtable_update(players_table, canonical["id"], merged)

    # Neutralize duplicates: clear telegram_user_id so they won't be matched again
    for rec in records_sorted[1:]:
        dup_update = {"telegram_user_id": ""}
        # clear link_token as well to avoid future accidental binds
        if (rec.get("fields") or {}).get("link_token"):
            dup_update["link_token"] = ""
        # set status to DUPLICATE if option exists; otherwise leave status untouched
        # (safe: Airtable will ignore invalid select values if not present? it will 422, so we don't set it)
        airtable_update(players_table, rec["id"], dup_update)
...
def upsert_player_by_telegram_user_id(players_table, telegram_user_id):
    ...
    if created.get("ok"):
            # DEDUP_TGID post-create: if concurrent creates happened, collapse to one canonical
            try:
                canon = dedup_players_by_telegram_user_id(players_table, tg)
                if canon:
                    return {"ok": True, "action": "dedup_canonical", "record_id": canon["id"]}
            except Exception:
                pass
            return {
            "ok": True,
